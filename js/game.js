...
function win() { 
	...
	eraseGhosts();
    // changed timeout to 1 from 1000 - CM edit
	setTimeout("prepareNextLevel()", 1);

}
function prepareNextLevel(i) { 
	if ( LEVEL_NEXT_TIMER === -1 ) { 
	    eraseBoardDoor();
        //changed interval to 1 from 250 - CM edit
		LEVEL_NEXT_TIMER = setInterval("prepareNextLevel()", 1);
	} else { 
		LEVEL_NEXT_STATE ++;
